describe Gateway::InterestingNumbersGateway do
  ...
  describe "#fetch" do
    ...
    it "returns a hash includes the correct name" do
      expect(response[:data][:name]).to eq "heat_pump_count_for_sap"
    end

    it "returns a hash includes the the data set", :aggregate_failures do
      expect(response[:data][:data].length).to eq 12
      expect(response[:data][:data].first).to eq({ monthYear: "2021-12", numEpcs: 929} )
      expect(response[:data][:data].select { |i| i[:monthYear] == "2022-02" }.first[:numEpcs]).to eq 1258
    end
