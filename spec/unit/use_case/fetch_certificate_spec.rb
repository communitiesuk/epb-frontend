describe UseCase::FetchCertificate do
  ...
  context "when there is a lodged certificate with the assessment_id" do
    let(:summary_gateway) do
      Gateway::AssessmentSummaryGateway.new(get_api_client)
    end
    ...
    let(:result) do
      {
        data: {
          ...
          heatingCostCurrent: "773",
          heatingCostPotential: "316",
    ...
    it "returns the certificate" do
      expect(fetch_certificate.execute("0000-0000-0000-0000-0666")).to eq(
        result,
      )
    end

    it "returns the certificate when the number includes leading or trailing whitespaces" do
      expect(fetch_certificate.execute(" 0000-0000-0000-0000-0666 ")).to eq(
        result,
      )
    end
  ...
  context "when there is a GONE certificate with an assessment_id" do
    let(:summary_gateway) do
      Gateway::AssessmentSummaryGateway.new(get_api_client)
    end
    let(:fetch_certificate) { described_class.new(summary_gateway) }
    ...
    before do
      FetchAssessmentSummary::GoneAssessmentStub.fetch(
        "0666-0000-0000-0000-0000",
      )
    end

    it "raises a 410 GONE error" do
      expect {
        fetch_certificate.execute("0666-0000-0000-0000-0000")
