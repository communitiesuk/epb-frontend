...
describe UseCase::FindNonDomesticAssessorByName do
  context "when there are no non domestic assessors for that name" do
  ...
  context "when there are non domestic assessors matched by the name" do
    ...
    it "returns list of non domestic assessors when the name includes leading or trailing whitespaces" do
      expect(
        find_assessor.execute(" Somewhatcommon   Name ")[:data][:assessors],
      ).to eq(valid_assessors)
    end
