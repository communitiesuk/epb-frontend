...
describe "Acceptance::AirConditioningInspectionCertificate", type: :feature do
  ...
  context "when an ac certificate exists" do
    ...
    it "shows the Assessment details section" do
      ...
      expect(response.body).to have_css "dd", text: "Not related to the owner/occupier or person who has technical control of the system or subcontractor."
    ...
    it "shows the subsystems inspected section" do
      ...
      expect(response.body).to have_css "h3", text: "VOL001/SYS001/CP001 Ground Floor Banking Hall, Interview Room and Cashiers Area"
      expect(response.body).to have_css "dt", text: "Description"
      expect(response.body).to have_css "dd", text: "Mitsubishi Electric PURY-P250LM-A1 VRF packaged system x 1 serve ceiling slot diffuser internal unit x 6"
  ...
  context "when an ac certificate expires" do
    ...
    it "shows the expired summary section" do
      expect(response.body).to have_css "dt", text: "This certificate expired on"
      expect(response.body).to have_css "dd", text: "21 May 2012"
    end
  end
  ...
  context "when an ac certificate is both NI and opted out" do
    ...
    it "removes assessor contact details" do
      ...
      expect(response.body).not_to have_css "dd", text: "123 My Street, My City, AB3 4CD"
      expect(response.body).to have_css "dt", text: "Accreditation scheme"
      expect(response.body).to have_css "dd", text: "Quidos"
      expect(response.body).to have_css "dt", text: "Accreditation scheme telephone"
      expect(response.body).to have_css "dd", text: "01225 667 570"
